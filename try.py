...
comando='curl -u admin:Mirgordomotica --digest "http://10.110.73.34/ISAPI/Streaming/Channels/101/picture" -o imagen_camara_' + str(ahora) + '.jpg --silent --show-error --max-time 10'
print(comando)
resultado = subprocess.run(comando, shell=True, capture_output=True, text=True)
pathfile = "C:\\Users\\amoraldo\\Downloads\\"
imgfile = "imagen_camara"
codigo = []
imgTotally = cv2.imread(pathfile + imgfile + ".jpg", cv2.IMREAD_GRAYSCALE)
#imgTotally = cv2.imread('imagen_camara_' + str(ahora) + '.jpg', cv2.IMREAD_GRAYSCALE)
 
cv2.imwrite('imagen_' + str(ahora) + ".jpg", imgTotally)
cv2.imshow("Escala_grises", imgTotally[100:270,1000:1200])
 
for indice, sizenum in enumerate([[1056,115,30,60],[1094,114,30,60],[1134,113,30,60],[1024,187,30,60],[1059,183,30,60],[1100,204,22,38]]):
  ...
  cv2.imwrite('imagen_recortada_' + str(indice+1) + "_" + str(ahora) + ".jpg", img)
